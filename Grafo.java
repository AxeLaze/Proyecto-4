...
//					System.out.println(S);
	 ...
	 public Grafo Kruskal_D() {
		 float CostoTotal=0;
		 Map<Integer, Vector<Integer>> Conjunto = new HashMap<Integer, Vector<Integer>>();			// Se crea un hashmap de vectores (conjuntos de nodos)
		 Grafo grafoKD = new Grafo(n,"Kruskal_D_"+this.name);
		 for(int k = 1; k <= nodo.size();k++) {														// Asignamos a cada conjunto un nodo, habra n conjuntos 
			 Vector<Integer> S = new Vector<Integer>();
			 S.add(k);
			 Conjunto.put(k,S);																		// Al hashmap Conjunto agregamos el conjuntok en la posicion k con su nodo k
			 Nodo s = nodo.get(k);
			 s.cnj = k;
			 grafoKD.nodo.put(k,s);
		 }
		 
		 int m = arista.size();
		 float[][] MatAux = new float [m][3];
		 int i = 0;
		 for(Map.Entry<Integer, Arista> entry:arista.entrySet()){      
		        Arista current = entry.getValue();
		        MatAux[i][0] = current.a;
		        MatAux[i][1] = current.b;
		        MatAux[i][2] = current.c;
		        i++;
		 }
		 // ORDENAMIENTO MENOR-MAYOR COSTO ARISTA
		  for (i = 0; i < m; i++) {
			  for(int j = 0; j < m-1; j++) {
				  if(MatAux[j][2] > MatAux[j+1][2]) {
					  float temp1 = MatAux[j][0];
					  float temp2 = MatAux[j][1];
					  float temp3 = MatAux[j][2];
					  MatAux[j][0] = MatAux [j+1][0];
					  MatAux[j][1] = MatAux [j+1][1];
					  MatAux[j][2] = MatAux [j+1][2];
					  MatAux[j+1][0] = temp1;
					  MatAux[j+1][1] = temp2;
					  MatAux[j+1][2] = temp3;
					  }
		  ...
		  for(i = 0; i < m; i++) {
			  int a = (int) MatAux[i][0]; int b = (int) MatAux[i][1];float c = MatAux[i][2];								// Nodo a y Nodo b Costo c
//			  System.out.println("Arista:"+a+"),"+b+"-("+c+")");
			  Nodo A = grafoKD.nodo.get(a);															// Obtenemos el nodo A del hashmap de nodos
			  Nodo B = grafoKD.nodo.get(b);															// Obtenemos al nodo B del hashmap de nodos
			  int ai = A.cnj; int bi = B.cnj;
			  if(ai != bi) {																	// Checamos que nodo A y B no pertenezcan al mismo conjunto		
				  CostoTotal = CostoTotal + MatAux[i][2];
//				  System.out.println("Iteracion "+i);
//				  System.out.println("Arista:"+a+"("+ai+"),"+b+"("+bi+")");
				  Arista C = new Arista((int) MatAux[i][0], (int) MatAux[i][1],MatAux[i][2]);		// Creamos la arista a,b con costo c
				  grafoKD.arista.put(i+1, C);														// Añadimos la arista al hashmap de las aristas
				  Vector<Integer> ConjuntoB = Conjunto.get(bi);										// Obtenemos el conjunto de B
				  Vector<Integer> ConjuntoA = Conjunto.get(ai);										// Obtenemos el conjunto de A
				  ListIterator<?> lit = ConjuntoB.listIterator();									
					while(lit.hasNext()) {															// Realizaremos un barrido de todos los nodos que pertenecen a B
						int x =  (int) lit.next();
						Nodo X = grafoKD.nodo.get(x);												// Creamos los nodos X que pertenecen a B
//						System.out.print(x+"("+X.cnj+"-->");
						X.cnj = ai;																	// Modoficamos su cnj, ahora perteneceran al conjunto A
//						System.out.println(X.cnj+")");
						ConjuntoA.add(x);															// Agregamos al conjunto A el elemento X que pertenecia a B
						grafoKD.nodo.put(x, X);														// Guardamos sus valores al hashmap de nodos
					}
//					System.out.println("Se convirtieron todos los b-->a");
//					System.out.println("El conjunto A"+ai+ConjuntoA);
//					System.out.println("");
			  }  
			  
		  }
		 System.out.println("Total del peso del árbol de expansión mínima resultante de Kruskal Directo: " + CostoTotal);
		 return(grafoKD);
	 }
	 
	 public Grafo Kruskal_I() {
		 float CostoTotal=0;
		 Grafo grafoKI = new Grafo(n,"Kruskal_I_"+this.name);
		 grafoKI.nodo = nodo;
		 ...
		 // ORDENAMIENTO MAYOR-MENOR COSTO ARISTA
		  for (i = m-1; i >= 0; i--) {
			  for(int j = m-1; j > 0; j--) {
				  if(MatAux[j][2] > MatAux[j-1][2]) {
					  float temp1 = MatAux[j][0];
		  ...
		  for(i = 0; i < m; i++) { 
			  Arista C = new Arista((int) MatAux[i][0], (int) MatAux[i][1],MatAux[i][2]);			// Creamos la arista a,b con costo c
			  grafoKI.arista.put(i+1, C);
//			  System.out.println(i+1+"  "+ C.a+"-"+C.b+"("+C.c+")");
		  }
		  ...
		  for(i = 1 ; i <= m; i++){ 
			  ...
			  for(int k = 1; k<=n; k++) {
				  ...
				  S.add(k);
				  Conjunto.put(k,S);		
				  Nodo K = grafoKI.nodo.get(k);
				  K.cnj = k;
				  grafoKI.nodo.put(k,K);
			  }
			  Arista current = grafoKI.arista.get(i);
			  int a = current.a; int b = current.b; float c = current.c;
//			  System.out.println("Revisamos la Arista "+a+","+b+"  para ser eliminada");
			  for(int j=i+1; j<=m; j++) {
				  Arista current2 = grafoKI.arista.get(j);
				  int a2 = current2.a; int b2 = current2.b;
				  Nodo A2 = grafoKI.nodo.get(a2);
				  Nodo B2 = grafoKI.nodo.get(b2);
				  int ai = A2.cnj; int bi = B2.cnj;
//				  System.out.print("Arista: "+a2+"("+A2.cnj+"),");
//				  System.out.print(b2+"("+B2.cnj+")");
//				  System.out.println();
				  grafoKI.nodo.put(b2,B2);
				  Vector<Integer> ConjuntoB = Conjunto.get(bi);										// Obtenemos el conjunto de B
				  Vector<Integer> ConjuntoA = Conjunto.get(ai);										// Obtenemos el conjunto de A
				  if(ai!=bi) {
				  ListIterator<?> lit = ConjuntoB.listIterator();									
					while(lit.hasNext()) {															// Realizaremos un barrido de todos los nodos que pertenecen a B
						int x =  (int) lit.next();
						Nodo X = grafoKI.nodo.get(x);												// Creamos los nodos X que pertenecen a B
//						System.out.print(x+"("+X.cnj+"-->");
						X.cnj = ai;																	// Modoficamos su cnj, ahora perteneceran al conjunto A
//						System.out.println(X.cnj+")");
						ConjuntoA.add(x);															// Agregamos al conjunto A el elemento X que pertenecia a B
						grafoKI.nodo.put(x, X);														// Guardamos sus valores al hashmap de nodos
					}
				  }
			  }
//			  System.out.println();
			  Nodo A = grafoKI.nodo.get(a);
			  Nodo B = grafoKI.nodo.get(b);
			  if(A.cnj == B.cnj) {
				  grafoKI.arista.remove(i,current);
//				  System.out.println(i+" removemos la arista "+ current.a+"-"+current.b+"("+current.c+")");

			  }
			  else {
				  CostoTotal = CostoTotal + c;
			  }
			  
			  
		  }
		 System.out.println("Total del peso del árbol de expansión mínima resultante de Kruskal Inverso: " + CostoTotal);
		 return(grafoKI);
	 }
	 
	 public Grafo Prim(int s) {
		 float CostoTotal=0;
		 int ct = 1;
		 Map<Integer, Arista> NextArista = new HashMap<Integer,Arista>();
		 Vector<Integer> S = new Vector<Integer>();			
		 Grafo grafoP = new Grafo(n,"Prim_"+this.name);
		 grafoP.nodo = this.nodo;
		 S.add(s);
		 Nodo Ss = grafoP.nodo.get(s);
		 Ss.cnj = s; 
		for(int i = 1; i <= n-1 ; i++) {												// n-1 aristas por agreagar, n-1 nodos agregar al conjunto S
			int contador = 1;
			ListIterator<?> lit = S.listIterator();									
			while(lit.hasNext()) {															// Realizaremos un barrido de todos los nodos que pertenecen a B
				int u =  (int) lit.next();
				Nodo U = grafoP.nodo.get(u);
				ListIterator<?> lit2 = U.conexionCosto.listIterator();
				while(lit2.hasNext()) {
					float[] AC =  (float[]) lit2.next();
					int v = (int) AC[0];
					Nodo V = grafoP.nodo.get(v);
					float c = AC[1];
//					System.out.println("Arista: "+u+","+v+"("+c+")");
					if(V.cnj != s) {  //Es decir, la arista u-v ya esta dentro de S
						Arista NewArista = new Arista(u,v,c);
						NextArista.put(contador,NewArista);
						contador++;
					}
				}
			}
		float menor = 99999;
		Arista Agregar = new Arista(0,0);
		for(Map.Entry<Integer, Arista> entry: NextArista.entrySet()){ 
			Arista current = entry.getValue();
//			System.out.println("Arista: "+current.a+","+current.b+","+current.c);
			float actual = current.c;
			if(menor > actual) {
				menor = actual;
				Agregar = current;
//				System.out.println("Arista menor: "+current.a+","+current.b+","+current.c);
				
			}
		}
		int Fa = Agregar.a; int Fb = Agregar.b; float Fc = Agregar.c;
		S.add(Fb);
//		System.out.println("Arista para agregar: "+Fa+","+Fb+","+Fc);
		grafoP.arista.put(ct,Agregar);
		ct++;
		CostoTotal = CostoTotal + Fc;
		Nodo Agregado = grafoP.nodo.get(Fb);
		Agregado.cnj = s;
		grafoP.nodo.put(Agregado.numero,Agregado);
		NextArista.clear();
		}
		 
		System.out.println("Total del peso del árbol de expansión mínima resultante de Prim: "+CostoTotal);
	 ...
	 public void CreateAristasCostos(float min,float max) {
		 ...
		 for(Map.Entry<Integer, Arista> entry:arista.entrySet()){
	        ...
	        if(Acosto.a!=Acosto.b) {
    			...
    			if(dirigido==false){ 
    				A.conexion.add(Acosto.b);
    				B.conexion.add(Acosto.a);
